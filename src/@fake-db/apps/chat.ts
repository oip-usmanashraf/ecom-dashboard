...
const data: { chats: any[]; contacts: any[]; profileUser: any } = {
  profileUser: {
    id: 11,
...
const reorderChats = (arr: any[], from: number, to: number) => {
...
mock.onGet('/apps/chat/chats-and-contacts').reply(() => {
  const chatsContacts = data.chats.map((chat: any) => {
    const contact = data?.contacts?.find((c: any) => c?.id === chat?.userId)
    // @ts-ignore
    contact?.chat = { id: chat.id, unseenMsgs: chat.unseenMsgs, lastMessage: chat.chat[chat.chat.length - 1] }
    return contact
  })

  const contactsToShow = data.contacts.filter((co: any) => {
    return !data.chats.some((ch: any) => {
      return co.id === ch.id
    })
  })
...
mock.onGet('/apps/chat/get-chat').reply(config => {
  ...
  const chat = data.chats.find((c: any) => c.id === userId)

  if (chat) chat.unseenMsgs = 0
  const contact = data?.contacts?.find((c: any) => c.id === userId)
  // @ts-ignore
...
mock.onPost('/apps/chat/send-msg').reply(config => {
  ...
  let activeChat = data.chats.find((chat: any) => chat.id === obj.contact.id)
